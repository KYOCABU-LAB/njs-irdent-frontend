...
const ToothDialog = ({ open, tooth, cara, onClose }: ToothDialogProps) => {
  const opcionesCara = useMemo(
    () =>
      tooth
        ? Array.from(new Set(getNormalizedMatrix(tooth.numero_dientes).flat()))
        : [],
    [tooth]
  );
      ...
      >
        <div className="flex items-center justify-between bg-sky-700 px-4 py-3 text-white">
          <h3 className="font-semibold">
            Detalle Diente {tooth.numero_dientes}
          </h3>
          <button
            onClick={onClose}
        ...
        <div className="p-6">
          ...
          <details className="rounded-lg border border-slate-200">
            ...
            <div className="px-4 pb-4 pt-2 text-sm text-slate-600">
              Aquí puedes listar cambios/observaciones previas del diente{" "}
              {tooth.numero_dientes}.
            </div>
          </details>
        </div>
      </motion.div>
